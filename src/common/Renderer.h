...
{
public:
    RenderSystem( GameEvents& gameEvents );
    ~RenderSystem();
...
private:
    ...
    EventConnections m_eventCons;
    GameEvents& m_gameEvents;
    TextureManager m_textureManager;
    AnimationManager m_animationManager;
    FontManager m_fontManager;

    typedef std::set<CompVisualTexture*> CompTextureSet;
    typedef std::set<CompVisualAnimation*> CompAnimationSet;
    typedef std::set<CompVisualMessage*> CompMessageSet;
    typedef std::pair<boost::shared_ptr<CompVisualMessage>, int> MessageAndTimeOut;
    typedef std::list<MessageAndTimeOut> CompMessageSetManaged;

    CompTextureSet m_textureComps;
    CompAnimationSet m_animComps;
    CompMessageSet m_msgComps;
    // comps generated by this system; not registered in CompMgr; int is time left for deletion (counted in update ticks)
    CompMessageSetManaged m_msgCompsManaged;

    void onRegisterComponent(Component& component);
    void onUnregisterComponent(Component& component);
    void onDispMessage(const std::string& message, int time);
